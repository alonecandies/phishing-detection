...
class Extractor():
    ...
    def web_traffic(url):
        try:
            queryUrl = url.replace("http://", "").replace("https://", "").split("/")[0]
            res = requests.get(
                "https://api.similarweb.com/v1/similar-rank/"+queryUrl+"/rank?api_key=e6910c792ae94adda6f12b2163c29ed6")
            # rank = int(res.json().similar_rank.rank)
            rankDetail = res.json()
            rank = int(rankDetail["similar_rank"]["rank"])
        except:
